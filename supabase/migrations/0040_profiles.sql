...
begin
    ...
    into v_id, v_user_id, v_updated_at, v_deleted_at
    from public.profiles
    where id = p_id;
    if v_id is null then
        -- do nothing
...
$$
select 'user:' || id
from ((select public.create_update_profile(
                      (json ->> 'id')::uuid,
                      (json ->> 'user_id')::uuid,
                      json ->> 'display_name',
                      p_timestamp,
      ...
      (select public.delete_profile(id, p_timestamp, p_last_pulled_at)
       from (select jsonb_array_elements_text(coalesce(p_changes -> 'deleted', '[]'::jsonb))::uuid) as deleted(id))) as users(id)
