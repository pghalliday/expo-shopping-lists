    ...
    if v_id is null then
        ...
        if v_deleted_at is null then
            if v_updated_at < p_last_pulled_at then
                ...
                raise exception 'create_update_profile: ProfileView with id (%) has been updated since the last pull', v_id;
            end if;
        else
            raise exception 'create_update_profile: ProfileView with id (%) has already been deleted', v_id;
        end if;
    end if;
end;
$$ language plpgsql;
    ...
    if v_id is null then
        ...
        if v_deleted_at is null then
            if v_updated_at < p_last_pulled_at then
                ...
                raise exception 'delete_profile: ProfileView with id (%) has been updated since the last pull', v_id;
            end if;
        end if;
    end if;
end;
...
begin
    perform realtime.broadcast_changes(
            'topic:' || coalesce(new.user_id, old.user_id)::text, -- topic - the topic to which we're broadcasting
